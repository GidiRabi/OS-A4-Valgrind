...
int main() {
    ...
    *singleton = 42;
    std::cout << "Value of singleton: " << **singleton << std::endl;

    Singleton<int>* anotherSingleton = Singleton<int>::getInstance();
    std::cout << "Value of anotherSingleton: " << **anotherSingleton << std::endl;

    Singleton<string>* stringSingleton = Singleton<string>::getInstance();
    *stringSingleton = "Hello, World!";
    std::cout << "Value of stringSingleton: " << **stringSingleton << std::endl;

    Singleton<string>* anotherStringSingleton = Singleton<string>::getInstance();
    std::cout << "Value of anotherStringSingleton: " << **anotherStringSingleton << std::endl; //returns the same  value as stringSingleton
